...
describe('authentication middleware', () => {
  const app = express().use(cookieParser()).get('/private', requireAuth, (req, res) => res.json(req.user)).use(errorHandler);
  it('rejects missing sessions', async () => expect((await request(app).get('/private')).status).toBe(401));
