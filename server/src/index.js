...
io.use((socket, next) => {
  try {
    const cookies = cookieParser.JSONCookies(Object.fromEntries((socket.handshake.headers.cookie || '').split(';').filter(Boolean).map((part) => { const index = part.indexOf('='); return [part.slice(0, index).trim(), decodeURIComponent(part.slice(index + 1))]; })));
    const payload = verifyAccessToken(sessionToken(cookies));
    socket.userId = payload.sub;
    next();
  } catch { next(new Error('Authentication required')); }
});
io.on('connection', (socket) => socket.on('scan:watch', async (scanId) => {
  if (typeof scanId !== 'string' || !/^[0-9a-f-]{36}$/i.test(scanId)) return;
  const owned = await db.scan.findFirst({ where: { id: scanId, userId: socket.userId }, select: { id: true } });
