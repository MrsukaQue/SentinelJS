    ...
    try {
      const result = await runScan(url, async (status, progress) => {
        await Promise.all([
          ...
          db.scan.update({ where: { id: scanId }, data: { status } }),
        ]);
      });
        ...
        ...(result.findings.length
          ? [
              db.finding.createMany({
                data: result.findings.map((finding) => ({ ...finding, scanId })),
              }),
            ]
          : []),
        db.scan.update({
...
worker.on('failed', (job, error) =>
  logger.error({ err: error, scanId: job?.data.scanId }, 'Scan job failed'),
);
