...
export function reportData(scan) {
  const counts = Object.fromEntries(['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFORMATIONAL'].map((severity) => [severity, scan.findings.filter((item) => item.severity === severity).length]));
  return {
    reportVersion: 1,
    ...
    score: scan.score,
    summary: { totalFindings: scan.findings.length, bySeverity: counts, notice: 'This score reflects limited passive checks and does not prove that the target is secure or vulnerable.' },
    findings: scan.findings.map(({ checkId, name, severity, category, evidence, description, impact, component, recommendation, reference, deduction }) => ({ checkId, name, severity, category, evidence, description, impact, component, recommendation, reference, deduction })),
  };
}

export function htmlReport(scan) {
  const report = reportData(scan);
  const findings = report.findings.map((item) => `<article><span class="severity ${item.severity.toLowerCase()}">${escapeHtml(item.severity)}</span><h2>${escapeHtml(item.name)}</h2><dl><dt>Evidence</dt><dd>${escapeHtml(item.evidence)}</dd><dt>Impact</dt><dd>${escapeHtml(item.impact)}</dd><dt>Affected component</dt><dd>${escapeHtml(item.component)}</dd><dt>Recommendation</dt><dd>${escapeHtml(item.recommendation)}</dd>${item.reference ? `<dt>Reference</dt><dd><a href="${escapeHtml(item.reference)}">${escapeHtml(item.reference)}</a></dd>` : ''}</dl></article>`).join('');
