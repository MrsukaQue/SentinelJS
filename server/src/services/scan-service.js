...
export async function createScan(userId, inputUrl) {
  ...
  const target = await db.target.upsert({
    ...
    create: { userId, url: url.href, hostname: url.hostname },
    update: {},
  });
  ...
  try {
    ...
    await db.scan.update({
      where: { id: scan.id },
      data: { status: 'FAILED', errorMessage: 'The scan could not be queued' },
    });
    throw error;
  }
}
...
export async function getScan(userId, id) {
  const scan = await db.scan.findFirst({
    where: { id, userId },
    include: { target: true, findings: { orderBy: { deduction: 'desc' } } },
  });
