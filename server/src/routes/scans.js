...
scansRouter.post(
  ...
  validate(
    z.object({ body: z.object({ url: z.string().min(1).max(2048) }), params: empty, query: empty }),
  ),
  asyncHandler(startScan),
);
scansRouter.get(
  '/',
  validate(
...
scansRouter.get(
  ...
  validate(
    z.object({
      body: empty,
      params: idParams,
      query: z.object({ format: z.enum(['json', 'html']).default('json') }),
    }),
  ),
