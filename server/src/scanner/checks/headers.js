...
const definitions = [
  {
    id: 'missing-csp', name: 'Content Security Policy is missing', header: 'content-security-policy', severity: 'HIGH',
  ...
  {
    id: 'missing-hsts', name: 'Strict Transport Security is missing', header: 'strict-transport-security', severity: 'MEDIUM', httpsOnly: true,
  ...
  {
    id: 'missing-nosniff', name: 'MIME sniffing protection is missing', header: 'x-content-type-options', severity: 'LOW', expected: 'nosniff',
  ...
  {
    id: 'missing-referrer-policy', name: 'Referrer Policy is missing', header: 'referrer-policy', severity: 'LOW',
  ...
  {
    id: 'missing-permissions-policy', name: 'Permissions Policy is missing', header: 'permissions-policy', severity: 'LOW',
...
export const headerCheck = {
  ...
  async run({ headers, finalUrl }) {
    const findings = definitions
      ...
      .filter((item) => !headers[item.header] || (item.expected && headers[item.header].toLowerCase() !== item.expected))
      .map((item) => finding({ ...item, category: 'headers' }, `${item.header} was ${headers[item.header] ? `set to “${headers[item.header]}”` : 'not present'}.`));

    const csp = headers['content-security-policy'] || '';
    if (!headers['x-frame-options'] && !/(^|;)\s*frame-ancestors\s+/i.test(csp)) {
      findings.push(finding({
        id: 'missing-frame-protection', name: 'Frame embedding protection is missing', category: 'headers', severity: 'MEDIUM',
        description: 'Neither X-Frame-Options nor a CSP frame-ancestors directive was observed.',
        impact: 'The page may be embeddable by another origin, increasing clickjacking exposure.',
