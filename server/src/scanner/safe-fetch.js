  ...
  return new Promise((resolve, reject) => {
    const request = transport.request(
      url,
      {
        method: 'GET',
        headers: {
          'User-Agent': `SentinelJS/${config.SCANNER_VERSION}`,
      ...
      (response) => {
          ...
          if (bytes > config.MAX_RESPONSE_BYTES)
            response.destroy(
              new AppError(
                413,
                'RESPONSE_TOO_LARGE',
                'Target response exceeded the scan size limit',
              ),
            );
        ...
        response.on('end', () => {
          ...
          for (let index = 0; index < rawHeaders.length; index += 2)
            if (rawHeaders[index].toLowerCase() === 'set-cookie')
              setCookies.push(rawHeaders[index + 1]);
          resolve({
    ...
    request.on('timeout', () =>
      request.destroy(
        new AppError(504, 'TARGET_TIMEOUT', 'The target did not respond before the timeout'),
      ),
    );
...
export async function safeFetch(input) {
  ...
  for (let redirects = 0; redirects <= config.MAX_REDIRECTS; redirects += 1) {
    ...
    if (![301, 302, 303, 307, 308].includes(response.status) || !response.headers.location)
      return { ...response, requestedUrl, finalUrl: currentUrl, redirectChain };
    if (redirects === config.MAX_REDIRECTS)
      throw new AppError(400, 'TOO_MANY_REDIRECTS', 'Target exceeded the redirect limit');
